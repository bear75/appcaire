...
export const clientSchema = pgTable(
  'clients',
  {
    ...
    encryptedLatitude: text('encrypted_latitude'),
    encryptedLongitude: text('encrypted_longitude'),
...
export const scheduleSchema = pgTable(
  'schedules',
  {
    ...
    eCareTaskId: text('ecare_task_id'),
    timefoldScore: json('timefold_score'),
    optimizationMetrics: json('optimization_metrics'),
    createdAt: timestamp('created_at', { mode: 'date' }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { mode: 'date' })
      .defaultNow()
      .$onUpdate(() => new Date())
...
export const visitRequirementSchema = pgTable(
  'visit_requirements',
  {
    ...
    eCareVisitType: text('ecare_visit_type'),
    eCareRequirements: json('ecare_requirements'),
    requiredEmployeeCount: integer('required_employee_count').default(1),
    createdAt: timestamp('created_at', { mode: 'date' }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { mode: 'date' })
      .defaultNow()
      .$onUpdate(() => new Date())
...
// Update the export to use the singular name
